...
function StatCounter({ value, label, suffix = "" }: { value: number; label: string; suffix?: string }) {
  ...
  return (
    <div ref={ref} className="text-center p-3 sm:p-4 rounded-2xl bg-white/40 border border-[#0D9488]/10 shadow-xs">
      <motion.p
        initial={{ opacity: 0, y: 15 }}
        animate={isInView ? { opacity: 1, y: 0 } : {}}
        transition={{ duration: 0.6 }}
        className="font-numeric font-bold text-3xl sm:text-5xl text-[#0D9488] tracking-tight"
      >
        {count}{suffix}
      </motion.p>
      <p className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-[#3D6060] mt-1.5">{label}</p>
    </div>
  );
}
...
export function AboutMe() {
  return (
    <section id="about" className="relative py-8 sm:py-10 lg:py-12 px-6 max-w-7xl mx-auto text-[#0D2626] overflow-hidden">
      ...
      >
        ...
        <h2 className="font-editorial text-3xl sm:text-5xl lg:text-6xl text-[#0D2626] mt-2 leading-tight">
      ...
      <motion.div
        ...
        viewport={{ once: true }}
        transition={{ duration: 0.6 }}
        className="mb-10 sm:mb-12 p-6 sm:p-8 rounded-3xl liquid-glass-card border border-[#0D9488]/15"
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
          {industries.map((ind, i) => {
            ...
            return (
              <div
                key={i}
                className="flex items-center gap-2.5 p-3 rounded-xl bg-white/60 border border-[#0D9488]/10 text-[#0D2626] shadow-2xs hover:border-[#0D9488]/30 hover:bg-white transition-all duration-200"
              >
                <div className="w-7 h-7 rounded-lg bg-[#0D9488]/10 text-[#0D9488] flex items-center justify-center flex-shrink-0">
                  <Icon className="w-3.5 h-3.5" />
                </div>
                <span className="text-xs font-semibold leading-snug">{ind.name}</span>
              </div>
            );
          })}
        </div>
      ...
      >
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-2.5 liquid-glass-dark px-8 py-3.5 text-xs font-bold uppercase tracking-widest text-white shadow-xl hover:scale-[1.02] transition-all duration-300"
        >
